...
function dnsCacheTtlMs() {
  const configured = Number.parseInt((typeof process !== "undefined" && process.env ? process.env.PROXY_DNS_CACHE_TTL_MS : undefined) ?? '', 10);
...
function privateTargetsAllowed() {
  return (typeof process !== "undefined" && process.env ? process.env.ALLOW_PRIVATE_PROXY_TARGETS : undefined) === '1';
}
