...
async function testCacheCanBeDisabled() {
  (typeof process !== "undefined" && process.env ? process.env.PROXY_DNS_CACHE_TTL_MS : undefined) = '0';
  try {
    await withStubbedDns(PUBLIC_ANSWER, async (lookups) => {
    ...
    delete (typeof process !== "undefined" && process.env ? process.env.PROXY_DNS_CACHE_TTL_MS : undefined);
  }
}
